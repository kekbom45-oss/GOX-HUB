--[[
    BAMBOO BOLT-X: ULTIMATE WARP (CLOSE DISTANCE UPDATE)
    - แก้ไข: วาร์ปไปทับตัวเป้าหมาย (No Offset)
...
-- [5] Main Menu Buttons
local btnSelect = createBtn("🎯 เลือกผู้เล่นเอง (ประชิดตัว)", Color3.fromRGB(30, 30, 30), mainMenu)
btnSelect.Position = UDim2.new(0, 0, 0.15, 0)

local btnRandom = createBtn("🎲 สุ่มวาร์ป (ประชิดตัว)", Color3.fromRGB(30, 30, 30), mainMenu)
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v ~= player then
            ...
            b.MouseButton1Click:Connect(function()
                ...
                statusText.Text = "วาร์ปทับตัว: " .. v.DisplayName; statusText.TextColor3 = Color3.fromRGB(0, 255, 150)
...
-- [8] Background Warp Loop (แก้ไขให้ไม่มี Offset)
task.spawn(function()
    while true do
        if warpMode == "Select" and selectedPlayer and selectedPlayer.Parent and selectedPlayer.Character then
            ...
            if hrp and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then 
                -- แก้ไขจุดนี้: ลบ * CFrame.new(0, 0, 2.5) ออกเพื่อให้วาร์ปทับตัว
        ...
        elseif warpMode == "Random" then
            ...
            if #targets > 0 then
                ...
                statusText.Text = "🎲 สุ่มวาร์ปทับ: " .. target.DisplayName
                local start = tick()
